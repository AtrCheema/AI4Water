...
ACTIVATIONS = {'LeakyRelu': lambda name='leaky_relu': layers.LeakyReLU(name=name),
               'PRelu': lambda name='p_relu': layers.PReLU(name=name),
...
class Model(AttributeNotSetYet):
    ...
    def __init__(self, data_config, nn_config, intervals=None):
    ...
    def build_nn(self, method='simple_lstm'):
        """
        :param method: either of 'simple_lstm', 'simple_cnn', 'cnn_lstm', 'lstm_cnn', 'input_attention',
                       'dual_attention', 'lstm_autoencoder,  'output_attention', 'tcn'
        :return:
        """
        self.nn_config['method'] = method

        setattr(self, 'method', method)

        self.k_model = getattr(self, 'build_' + method + '_model')()
    ...
    def train_nn(self, st=0, en=None, indices=None, tensorboard=None):
        ...
        elif self.method == 'input_attention':
            history = self.fit([train_x, s0_train, h0_train], train_label, callbacks)

        elif self.method in ['lstm_cnn', 'simple_lstm', 'cnn_lstm', 'simple_cnn', 'tcn']:
    ...
    def predict(self, st=0, ende=None, indices=None):
        ...
        elif self.method in ['lstm_cnn', 'simple_lstm', 'cnn_lstm', 'lstm_autoencoder', 'tcn']:
        ...
        errors = FindErrors(test_label, predicted)
        for er in ['mse', 'rmse', 'r2', 'nse', 'kge', 'rsr', 'percent_bias']:
            print(er, getattr(errors, er)())

        plot_results(test_label, predicted, str(st) + '_' + str(ende))
    ...
    def build_tcn_model(self):
        ...
        k_model = self.compile(inputs, predictions)
        tcn_full_summary(k_model, expand_residual_blocks=True)

        return k_model

    def build_lstm_autoencoder_model(self, method='prediction'):
        """
        ...
        k_model = self.compile(inputs, outputs)
    ...
    def build_lstm_cnn_model(self):
        ...
        # lstm_layer = layers.LSTM(lstm['lstm_units'],
        ...
        #
        # activations = ACTIVATIONS[lstm['lstm_act']](name='lstm_act')(lstm_layer)
    ...
    def build_simple_cnn_model(self):
    ...
    def compile(self, model_inputs, outputs):
        ...
        k_model = KModel(inputs=model_inputs, outputs=outputs)
    ...
    def build_output_attention_model(self):
        ...
        enc_lstm_out = layers.LSTM(self.nn_config['lstm_units'], return_sequences=True,
    ...
    def build_input_attention_model(self):
        ...
        k_model = KModel(inputs=[enc_input, s0, h0], outputs=predictions)
        adam = keras.optimizers.Adam(lr=self.nn_config['lr'])
        k_model.compile(loss=self.nn_config['loss'], optimizer=adam, metrics=['mse'])
        k_model.summary()
    ...
    def build_dual_attention_model(self):
        ...
        k_model = KModel(inputs=[enc_input, input_y, s0, h0, s_de0, h_de0], outputs=output)
        adam = keras.optimizers.Adam(lr=self.nn_config['lr'])
        k_model.compile(loss=self.nn_config['loss'], optimizer=adam, metrics=['mse'])
        k_model.summary()
