    ...
    def test_model_predict(self):
        ...
        for x_trans in ["minmax", "box-cox", "robust", "power", "center", "zscore", "scale", "quantile"]:
            for y_trans in ["log", "sqrt", "log10", "log2"]:
                ...
                model.fit()
                t,p = model.predict('training', return_true=True, process_results=False)
                ...
                model2 = Model(model="RandomForestRegressor", data=df2,
                               verbosity=0,
                               val_fraction=0.0, test_fraction=0.0,)
                model2.fit()
                t2, p2 = model2.predict('training', return_true=True, process_results=False)
